<!--数据采集:列表-->
<template>
    <div class="audit-data-collection-content" style="">
        <!--顶部-->
        <div class="task-header">
            ...
            <div class="task-btn-group" v-show="taskListIsNull">
                ...
                <!--全部取消|全部选择-->
                <Button class="btn-1" type="primary" size="small" @click.native="batchSelect"> {{ selectAllText }} </Button>
                <!--批量通过-->
                <Button class="btn-2" type="primary" size="small" @click.native="batchPass" v-if="selectedTask.length > 0">{{$t('tool_batch_pass')}}</Button>
                <!--批量驳回-->
                <Button class="btn-3" type="primary" size="small" @click.native="taskWillReject('', true)" v-if="selectedTask.length > 0">{{$t('tool_batch_rejection')}}</Button>
                <!--批量重置-->
                <Button class="btn-4" type="primary" size="small" @click.native="taskWillReset('', true)" v-if="selectedTask.length > 0">{{$t('tool_bulk_reset')}}</Button>
                <!--批量挂起-->
                <Button class="btn-5" type="warning" size="small" @click.native="taskWillSetDifficult" v-if="selectedTask.length > 0">{{$t('tool_batch_troublesome_work')}}</Button>
                <!--清除并退出-->
                <Button class="btn-6" type="error" size="small" @click="exitfirmModal = true">{{$t('tool_clear_and_exit')}}</Button>
        ...
        <div class="audit-wrapper">
            <div v-show="!taskListIsNull" class="box-1831" style="">
            ...
            <div class="audit-content" v-show="taskListIsNull">
                ...
                <CheckboxGroup v-model="selectedTask" class="small" v-if="taskListCopy.length">
                    <div class="audit-item" v-for="(task, index) in taskListCopy" :key="index">
                        <Spin size="small" class="image-loading" v-if="!(task.data.id)">
                            <Icon type="ios-loading" size=18 class="spin-icon-load"></Icon>
                            <div class="load-4">Loading</div>
                        </Spin>
                        <div class="audit-result" @click="imageViewer(task.data.id, index, $event)">                            
                            <span class="info-1851">{{$t('tool_collection_information_ID')}}：{{task.data.id}}</span>
                        </div>
                        <div class="audit-button" v-if="task.data.id">
                            <!--通过-->
                            ...
                            <!--驳回-->
                            <Button type="error" size="small" style="margin-left:2px;"
                                @click.native="taskWillReject(task.data.id)">{{$t('tool_reject')}}</Button>
                            <!--重置-->
                            <Button size="small" style="margin-left:2px;"
                                @click.native="taskWillReset(task.data.id)">{{$t('tool_reset')}}</Button>
                            <!--挂起-->
                            <Button size="small" type="warning" style="margin-left:2px;"
                        ...
                        <div class="audit-result-info">
                            ...
                            <Tooltip :transfer="true" placement="top-start" style="margin-right:20px" v-if="task.workResult.feedback">
                                ...
                                <ErrorTaskReasonShow slot="content" :reason="task.workResult.feedback"></ErrorTaskReasonShow>
                            </Tooltip>
                        </div>
                    </div>
                </CheckboxGroup>
        ...
        <Spin fix v-if="loading" class="load-3">{{loadingText}}</Spin>
        ...
        <Modal v-model="editModal"
            ...
            <Spin fix v-if="loading" class="load-2">{{loadingText}}</Spin>
        </Modal>
...
export default {
    ...
    watch: {
        taskList (newV, oldV) {
            ...
            this.loading = false;
            if (newV.length === 0) {
                this.messageShowFun({type:'warning', content:this.$t('tool_no_jobs'), duration:3});
    ...
    methods: {
        ...
        showEditModal (index) {
            if (this.taskIsTimeOut) {
                this.messageShowFun({type:'warning', content:this.$t('tool_timed_out'), duration:2});               
                return;
            }
            this.editModal = true;
        ...
        handleModalOnOk () {
            ...
            if (this.rejectReason.trim() === '') {
                this.messageShowFun({type:'error', content:this.$t('tool_reason_empty'), duration:2});               
                return;
            }
        ...
        editModalVisibleChange (flag) { // true 打开 false 关闭
            if (flag) {
                this.$nextTick(() => {
                    var res = this.taskListCopy[this.currentIndex];
                    var result = res.dataResult.result || res.workResult.result;
                    setTimeout(() => {
                ...
                success: (res) => {
                    ...
                    if (res.error) {
                        ...
                        if (resource.length === 0) {
                            this.messageShowFun({type:'error', content:this.$t('tool_request_failed'), duration:2});                           
                            return;
                        }
                        let src;
                        let labelList = [];
        ...
        submitEditTask () {
            if (this.taskIsTimeOut) {
                this.messageShowFun({type:'warning', content:this.$t('tool_timed_out'), duration:3});               
                return;
            }
            if (this.loading) {
                return;
            ...
            $.ajax({
                ...
                success: (res) => {
                    this.loading = false;
                    if (res.error) {
                        this.messageShowFun({type:'warning', content:res.message, duration:2});                        
                    } else {
    ...
    components: {
        ...
        'data-collection-audit-view': () => import('components/task-audit-view/data-collection-audit-view'),
        'task-info': TaskInfo,
...
<style>
.audit-data-collection-content{position:relative; min-height: 100%;}
.box-1831{height: 200px;background: #eeeeee;font-size: 26px;text-align:center;line-height:200px;}
.info-1851{display: block;height: 100%;width: 100%;cursor: zoom-in;padding-top: 1em;}
</style>