    ...
    <div class="" style="position:relative; height: 100%;">
        <div class="task-header" id="task-produce">
            ...
            <div class="task-btn-group" v-show="taskListIsNull">
                ...
                <!--<Button type="error" @click.native="taskWillReset">重置</Button>-->
                <!-- <Button type="warning" @click.native="taskSetDifficult">{{$t('tool_diffcult_job')}}</Button> -->
                <Button type="default" @click.native="exit">{{$t('tool_quit')}}</Button>
...
<script>
    ...
    export default {
        ...
        methods: {
                ...
                $.ajax({
                    ...
                    success: (res) => {
                        ...
                        if (res.error) {
                            ...
                            let result = this.taskList[this.currentTaskIndex].dataResult.result;
            ...
            // 审核重置
            taskReset (reason) {
                if (this.taskIsTimeOut || this.loading) { // 没有批量操作才可以这样
                    return;
                }
                let dataId = this.dataId;
                let result = {};
