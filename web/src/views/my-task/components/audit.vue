...
<template>
    <div>
        ...
        <Row>
            <i-col span="8" class="padding-bottom-30" v-for="(item,index) in carInfo" :key="index" >
                    ...
                    <!-- <Row v-if="Object.keys(labelTypeList).length > 0">
                        <i-col span="3" class="card-info" v-for="(item,value,index) in labelTypeList" :key="index">
                            <Tooltip placement="top-start">
                                <div slot="content">
                                    <p>{{item.title}}</p>
                                    <p class="toolTip">已提交：{{item.value.submit_count}}</p>
                                    <p class="toolTip">已通过：{{item.value.allowed_count}}</p>
                                    <p>删除操作记录(仅展示)</p>
                                    <p class="toolTip">删除框：{{item.value.delete_count}}</p>
                                </div>
                                <p style="color:#999;">{{item.title}}</p>
                                <p style="font-size:30px;" class="padding-top-13">{{item.value ? (item.value.submit_count * 1 + item.value.effective_count * 1) : 0}}</p>
                            </Tooltip>
                        </i-col>
                    </Row> -->
                    <Row>
        ...
        <Modal
            ...
            <Row>
                <div style="margin:0px 0px 20px 15px;">
                    <Button type="primary" @click="getSearch" style="margin-right:20px;">{{$t('operator_query')}}</Button>
            ...
            this.carInfo =  [
                {
                    ...
                    dasc: [
                        this.$t('operator_audit_submit_total_time') + '：' + this.$t('operator_audit_submit_total_time_dasc'),
                        this.$t('operator_audit_submit_total_folio') + '：' + this.$t('operator_audit_submit_total_folio_dasc')
                    ],
                    info: [
                        {
                            name: this.$t('operator_audit_submit_total_folio'),
                            num: this.sumData.work_count ? this.sumData.work_count : 0,
                            skip: 0,
                            hint: 0,
                            poptip:[],
                            status: "-1"
                        ...
                        {
                            name: this.$t('operator_audit_submit_total_time'),
                            num: this.sumData.submit_count ? this.sumData.submit_count : 0,
                            skip: 1,
                            hint: 0,
                            poptip:[],
                            status: "3"
                ...
                },
                {
                    ...
                    dasc: [
                        this.$t('operator_reject_count') + '：' + this.$t('operator_reject_count_dasc'),
                        this.$t('operator_reset_count') + '：' + this.$t('operator_reset_count_dasc'),
                        this.$t('operator_rejected_count') + '：' + this.$t('operator_rejected_count_dasc')
                    ],
                    info: [
                        {
                            name: this.$t('operator_reject_count'),
                            num: this.sumData.allowed_count ? this.sumData.refuse_count : 0,
                            skip: 0,
                            hint: 0,
                            poptip:[],
                            status: "-1"
                        ...
                        {
                            name: this.$t('operator_rejected_count'),
                            num: this.sumData.refused_count ? this.sumData.refused_count : 0,
                            skip: 0,
                            hint: 0,
                            poptip:[],
                            status: "-1"
                        ...
                        {
                            name: this.$t('operator_reset_count'),
                            num: this.sumData.reseted_count ? this.sumData.reset_count : 0,
                            skip: 0,
                            hint: 0,
                            poptip:[],
                            status: "-1"
                ...
                },
                {
                    ...
                    dasc: [
                        this.$t('tool_percent_pass') + '：' + this.$t('operator_percent_pass_dasc'),
                        this.$t('operator_work_time') + '：' + this.$t('operator_work_time_dasc')
                    ],
                    info: [
                        {
                            name: this.$t('tool_percent_pass'),
                            num: this.sumData.pass_rate ?  (this.sumData.pass_rate + "%") : "0.00%",
                            skip: 0,
                            hint: 0,
                            poptip:[],
                            status: "-1"
                        ...
                        {
                            name: this.$t('operator_work_total_time'),
                            num: this.sumData.work_time ? this.sumData.work_time : 0,
                            skip: 0,
                            hint: 0,
                            poptip:[],
                            status: "-1"
                ...
                //     title: "作业统计",
                //     dasc: [
                //         "无效数据总数：当作业员在清洗控件选择NO记录张数总和",
                //         "有效数据总数：当作业员在清洗控件选择yes记录张数总和"
                //     ],
                //     info: [
                //        {
                //            name: "无效数据总数",
                //            num: Object.keys(this.sumData).length ? (this.sumData.invalid_data_allowed_count * 1 + this.sumData.invalid_data_count * 1) : 0,
                //            skip: 0,
                //            hint: 1,
                //            poptip:[]
                //        },
                //        {
                //            name: "有效数据总数",
                //            num: Object.keys(this.sumData).length ? (this.sumData.valid_data_allowed_count * 1 + this.sumData.valid_data_count * 1) : 0,
                //            skip: 0,
                //            hint: 1,
                //            poptip:[]
                //        }
            ...
            carInfo: [
                ...
                },
                {
                    ...
                    info: [
                        {
                            name: this.$t('tool_percent_pass'),
                            num: this.sumData.pass_rate ?  (this.sumData.pass_rate + "%") : "0%",
                            skip: 0,
                            hint: 0,
                            poptip:[],
                            status: "-1"
            ...
            $.ajax({
                ...
                success: (res) => {
                    ...
                    if (res.error) {
                        ...
                        this.statuses = res.data.audit_statuses;
