            ...
            <Table
                ...
                <div slot="footer">
                    <!-- <Button type="primary" style="margin-left:10px" @click="batchRefuse" :disabled="selItem.length == 0"
                            :loading="batchRefuseLoading">{{$t('admin_batch_rejected')}}
                    </Button> -->
                    <Button type="primary" style="margin-left:10px" @click="batchReset" :disabled="selItem.length == 0"
                            :loading="batchResetLoading">{{$t('admin_batch_reset')}}
                    </Button>
...
<script>
    ...
    export default {
        ...
        data () {
            return {
                ...
                tableOption: [
                    ...
                    //     maxWidth: 120,
                    //     render: (h, para) => {
                    //         return h('span', {}, para.row.end_time * 1 - para.row.start_time * 1);
                    //     },
                    ...
                    //                         content: () => {
                    //                             return h('span', {}, [
                    //                                 h('div', this.$t('admin_time_unit')),
                    //                             ]);
                    //                         }
                    //                     }
                    //                 }, [
                    //                     h('span', this.$t('admin_cumulative')),
                    //                     h('Icon', {
                    //                         style: {
                    //                             marginLeft: '3px',
                    //                             marginTop: '1px',
                    ...
                    // },
                    {
                        title: this.$t('admin_submission_time'),
                        key: 'end_time',
                        sortable: 'custom',
                        align: 'center',
                        maxWidth: 140,
        ...
        methods: {
            initTask (project_id) {
                this.getworkResource(this.dataId, project_id);
                this.textModal = true;
            },
            ...
            getRecord (data_id, project_id) {
                ...
                $.ajax({
                    ...
                    data: {
                        access_token: this.$store.state.user.userInfo.accessToken,
                        // task_id: this.$route.params.id,// 管理员界面，不传
