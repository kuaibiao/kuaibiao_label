<template>
    <div class="module-editor">
        ...
        <div class="editor-item">
            <h4 class="editor-header">{{$t('template_multi_input_maxRows')}}</h4>
            <InputNumber v-model="module.maxRows"
                         :min="1"
                   @on-change="saveChange"/>
        </div>
        <div class="editor-item">
            <Checkbox v-model="module.required"
                      @on-change="saveChange">{{$t('template_whether_must_required')}}
            </Checkbox>
        </div>
    </div>
</template>
