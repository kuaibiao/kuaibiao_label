    ...
    <div class="image-audit-view">
        <div class="result-view-header">
            ...
            <div>
                ...
                <Button type="warning" size="small" @click="errorTask('reject')" :loading="loading">{{$t('tool_w')}}(W)
                </Button>
                <Button type="info" size="small" v-if="needEdit" :disabled="taskListCopy[currentIndex] && taskListCopy[currentIndex].imageLoadError" @click="editResult" :loading="loading">
                    {{$t('tool_modify_space')}}
                </Button>
                <Button type="primary" size="small" @click="taskPass" :loading="loading">{{$t('tool_d')}} (D)</Button>
            </div>
        </div>
...
<script>
    ...
    import VideoTrackingComponent from '../../video-tracking/video-tracking';
    import EventBus from '@/common/event-bus';
    const VideoTrackingCtor = Vue.extend(VideoTrackingComponent);

    const ImageLabelInstance = null;
    export default {
        name: "video-tracking-audit-view",
        props: {
            taskList: {
                type: Array,
                require: true,
                    ...
                    success: (res) => {
                        ...
                        if (res.error) {
                            ...
                            if (this.taskListCopy[index]) {
                                this.taskListCopy[index].rawImage = resource[0][1];
                            }

                            this.currentSrc = resource[0][1];
