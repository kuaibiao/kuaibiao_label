        ...
        methods: {
            ...
            updatePointCloudViewer (index) {
                ...
                let data = this.result.data || [];
                 if (this.categoryView === 'pointcloud_segment' && data[0] && typeof data[0].indexs === 'string') {
                    ...
                    $.getJSON(cameraConfigUrl.url, (cameraConfig) => {
                        Object.keys(urls).map((key) => {
                            if (key.indexOf('3d_img') === 0) {
                                image2dList.push({
                                    url: urls[key].url,
                                    camera_internal: cameraConfig[key].camera_internal,
                                    camera_external: cameraConfig[key].camera_external,
                                    distortion: cameraConfig[key].distortion,
                                    box_type: cameraConfig[key].box_type,
                                });
                            }
                        });
                        this.$nextTick(() => {
