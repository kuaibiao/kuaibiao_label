...
class Cache extends \yii\caching\Cache
{
    ...
    {
        return $this->redis->msetex($data, $expire);
        /*
        $failedKeys = [];
        if ($expire == 0) {
            $this->redis->msetex($data, $expire);
        } else {
            $expire = (int)$expire;
            $this->redis->multi();
    ...
    public function getOrSet($key, $callable, $expire = 1800, $resetCache = null)
    {
        $key = $this->buildKey($key);
        return $this->redis->getOrSet($key, $callable, $expire, $resetCache);
    }
}
