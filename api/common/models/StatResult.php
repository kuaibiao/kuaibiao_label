 ...
 *
 */
class StatResult extends ModelComponent
{
	...
	const TYPE_PENLINE_COUNT = 16; //标注铅笔总线数
	...
	public static $tableName = 'stat_result';
	...
	public static function updateCounter($where = [], $counters = [], $lockTimes = 0)
	{
        ...
        $statResult = self::find()
        ->where(['project_id' => $projectId, 'task_id' => $taskId, 'type' => $type, 'action' => $action])
        ->asArray()->limit(1)->one();
        if (!$statResult)
        {
            //并发锁, 防止N多人同时第一次点击执行任务
            $cacheKey = sprintf('statResult.updateCounter.%s.%s.%s.%s.lock', $projectId, $taskId, $type, $action);
            ...
            //-----------------------------------
            $statResult = new self();
            $statResult->project_id = $projectId;
            $statResult->task_id = $taskId;
            $statResult->type = $type;
            $statResult->action = $action;
            $statResult->value = 0;
            $statResult->save();
            
            $statResult = $statResult->getAttributes();
        ...
        if ($counters)
        {
            ...
            {
                if ($v < 0 && isset($statResult[$k]) && ($statResult[$k] + $v < 0))
                {
                    $_logs['$counters.$k'] = $k;
                    $_logs['$counters.$v'] = $v;
                    $_logs['$statResult.$v'] = $statResult[$k];
                    Yii::error(__CLASS__.' '.__FUNCTION__.' '.__LINE__.' $statResult value <0 '.json_encode($_logs));
            ...
            self::updateAllCounters($counters, ['id' => $statResult['id']]);
        }
	...
	public static function fetchData($param = [])
	{
		...
		//查询
		$statResults = self::find()
			->select(['task_id', 'type', 'action', 'SUM(value) as total'])
			->where(['project_id' => $projectId])
			->andWhere(['in', 'task_id', $taskIds])
			->andWhere(['!=', 'value', 0])
			->groupBy(['task_id', 'type', 'action'])
			->asArray()
			->all();
		...
		{
			$resultsByTask[$results['task_id']][] = $results; //按照task_id分组
		}
		foreach($resultsByTask as $taskId => $results)
		{
			$resultsByType = [];
			...
			foreach($allTypes as $type)
			{
				...
				{
					$statData[$taskId][$typeName] = ['effective_count' => 0, 'allowed_count' => 0, 'toaudit_count' => 0, 'delete_count' => 0];
				}
				else //有效数据=已被通过+待审核
				{
					$submitTotal = 0; //提交总数
					...
					$statData[$taskId][$typeName] = ['effective_count' => $effectiveTotal, 'allowed_count' => $allowedTotal, 'toaudit_count' => $toAuditTotal, 'delete_count' => $deleteTotal];
				}
			}
