 ...
 *
 */
class StatResultUser extends ModelComponent
{
	...
	public static $tableName = 'stat_result_user';
	...
	public static function updateCounter($where = [], $counters = [], $lockTimes = 0)
	{
        ...
        $statResultUser = self::find()
        ->where(['project_id' => $projectId, 'task_id' => $taskId, 'user_id' => $userId, 'type' => $type, 'action' => $action])
        ->asArray()->limit(1)->one();
        if (!$statResultUser)
        {
            //并发锁, 防止N多人同时第一次点击执行任务
            $cacheKey = sprintf('statResultUser.updateCounter.%s.%s.%s.%s.%s.lock', $projectId, $taskId, $userId, $type, $action);
            ...
            //-----------------------------------
            $statResultUser = new self();
            $statResultUser->project_id = $projectId;
            $statResultUser->task_id = $taskId;
            $statResultUser->user_id = $userId;
            $statResultUser->type = $type;
            $statResultUser->action = $action;
            $statResultUser->value = 0;
            $statResultUser->save();
            
            $statResultUser = $statResultUser->getAttributes();
        ...
        if ($counters)
        {
            ...
            {
                if ($v < 0 && isset($statResultUser[$k]) && ($statResultUser[$k] + $v < 0))
                {
                    $_logs['$counters.$k'] = $k;
                    $_logs['$counters.$v'] = $v;
                    $_logs['$statResultUser.$v'] = $statResultUser[$k];
                    Yii::error(__CLASS__.' '.__FUNCTION__.' '.__LINE__.' $statResultUser value <0 '.json_encode($_logs));
            ...
            self::updateAllCounters($counters, ['id' => $statResultUser['id']]);
        }
	...
	public static function fetchData($param = [])
	{
		...
		$query = self::find()
			->select(['user_id', 'type', 'action', 'SUM(value) as total'])
			->where(['project_id' => $projectId]);
		if(!empty($param['user_id']))
		{
			$userIds = is_array($param['user_id']) ? $param['user_id'] : [$param['user_id']];
		...
		{
			$resultsByUser[$results['user_id']][] = $results; //按照task_id分组
		}
		foreach($resultsByUser as $userId => $results)
		{
			...
			foreach($allTypes as $type)
			{
				...
				{
					$statData[$userId][$typeName] = ['effective_count' => 0, 'allowed_count' => 0, 'toaudit_count' => 0, 'delete_count' => 0];
				}
				else //有效数据=已被通过+待审核
				{
					$submitTotal = 0; //提交总数
					...
					$statData[$userId][$typeName] = ['effective_count' => $effectiveTotal, 'allowed_count' => $allowedTotal, 'toaudit_count' => $toAuditTotal, 'delete_count' => $deleteTotal];
				}
			}
