 ...
 *
 */
class StatResultData extends ModelComponent
{
	...
	const TYPE_LABEL_UNKNOWN_COUNT = 24; //被标记为Unknown的数据数
	...
	public static $tableName = 'stat_result_data';
	...
	public static function updateCounter($where = [], $counters = [], $lockTimes = 0)
	{
        ...
        $statResultData = self::find()
        ->where(['project_id' => $projectId, 'task_id' => $taskId, 'data_id' => $dataId, 'type' => $type, 'action' => $action])
        ->asArray()->limit(1)->one();
        if (!$statResultData)
        {
            //并发锁, 防止N多人同时第一次点击执行任务
            $cacheKey = sprintf('statResultData.updateCounter.%s.%s.%s.%s.%s.lock', $projectId, $taskId, $dataId, $type, $action);
            ...
            //-----------------------------------
            $statResultData = new self();
            $statResultData->project_id = $projectId;
            $statResultData->task_id = $taskId;
            $statResultData->data_id = $dataId;
            $statResultData->type = $type;
            $statResultData->action = $action;
            $statResultData->value = 0;
            $statResultData->save();
            
            $statResultData = $statResultData->getAttributes();
        ...
        if ($counters)
        {
            ...
            {
                if ($v < 0 && isset($statResultData[$k]) && ($statResultData[$k] + $v < 0))
                {
                    $_logs['$counters.$k'] = $k;
                    $_logs['$counters.$v'] = $v;
                    $_logs['$statResultData.$v'] = $statResultData[$k];
                    Yii::error(__CLASS__.' '.__FUNCTION__.' '.__LINE__.' $statResultData value <0 '.json_encode($_logs));
            ...
            self::updateAllCounters($counters, ['id' => $statResultData['id']]);
        }
	...
	public static function fetchData($param = [])
	{
		...
		$query = self::find()
			->select(['data_id', 'type', 'action', 'SUM(value) as total'])
			->where(['project_id' => $projectId]);
		if(!empty($param['data_id']))
		{
			$dataIds = is_array($param['data_id']) ? $param['data_id'] : [$param['data_id']];
		...
		{
			$resultsByData[$results['data_id']][] = $results; //按照data_id分组
		}
		foreach($resultsByData as $dataId => $results)
		{
			$resultsByType = [];
			...
			foreach($allTypes as $type)
			{
				...
				{
					$statData[$dataId][$typeName] = ['effective_count' => 0, 'allowed_count' => 0, 'toaudit_count' => 0, 'delete_count' => 0];
				}
				else //有效数据=已被通过+待审核
				{
					$submitTotal = 0; //提交总数
					...
					foreach($resultsByType[$typeName] as $result)
					{
						...
						else if(in_array($result['action'], [self::ACTION_REFUSED, self::ACTION_RESETED]))
						{
					...
					$statData[$dataId][$typeName] = ['effective_count' => $effectiveTotal, 'allowed_count' => $allowedTotal, 'toaudit_count' => $toAuditTotal, 'delete_count' => $deleteTotal];
				}
			}
