...
/**
 *
 * 2D视频连续帧json格式输出 -  json（2D一帧一结果）
 *
 *
 */
class JsonFrameStack extends Component
{
    ...
    public static function run($args)
    {
        ...
        if(count($batchIds) > 3){
            $batchDirname = Yii::t('app', 'pack_script_video_JsonFrameStack').'_'.$projectId.'_more_'.date('YmdHis');
        }else{
            $batchDirname = Yii::t('app', 'pack_script_video_JsonFrameStack').'_'.$projectId.'_'.implode(',', $batchIds).'_'.date('YmdHis');
        }
            ...
            foreach ($list as $k => $v)
            {
                ...
                if (is_array($resultData) && count($resultData)) {
                    ...
                    foreach($images as $k_i => $image_path){
                        ...
                        $datas['image_url'] = $image_path;
                        $results = [];
                        $results['data'] = $resultData_image;
                        $text = [];
