<?php
namespace common\components\exportHandler\video;
...
/**
 *
 * 2D视频连续帧mark图输出 -  mark图有标签(2D一帧一结果）
 *
 *
 */
class MarkJpgHasLabelFrameStack extends Component
{
    ...
    public static function run($args)
    {
        ...
        if(count($batchIds) > 3){
            $batchDirname = Yii::t('app', 'pack_script_video_MarkJpgHasLabelFrameStack').'_'.$projectId.'_more_'.date('YmdHis');
        }else{
            $batchDirname = Yii::t('app', 'pack_script_video_MarkJpgHasLabelFrameStack').'_'.$projectId.'_'.implode(',', $batchIds).'_'.date('YmdHis');
        }
            ...
            foreach ($list as $k => $v)
            {
                ...
                if (is_array($resultData) && count($resultData)) {
                    ...
                    if(isset($resultData[0]['frame'])){
                        //新格式数据转成就格式
                        $tmp_list = [];
                        foreach($resultData as $r_k => $r_v){
                            if(!isset($tmp_list[$r_v['frame']])){
                                $tmp_list[$r_v['frame']] = [
                                    'index' => $r_v['frame'],
                                    'result' => []
                                ];
                            }
                            $tmp_list[$r_v['frame']]['result'][] = $r_v;
                        }
                        ksort($tmp_list);
                        $resultData = $tmp_list;
                    }
                    // 循环处理作业中的每一帧
                    $images = current($dataArr);
                    foreach($images as $k_i => $image_path){
                        ...
                        if(isset($configs['RemoveEmpty']) && $configs['RemoveEmpty'] && (!isset($resultData[$k_i]['result'])||!count($resultData[$k_i]['result']))){
                            $checkMessages['error'][$v['data_id']][$fileInfo['basename']]= Yii::t('app', 'pack_data_no_result' );
                            $noDataResult[] = $v['data_id'];
                            continue;
                        }
                        $resultData_image = $resultData[$k_i]['result'];
                        $datas['frameNumber'] = $k_i;
                        foreach ($resultData_image as $k_l => $label) {
                            if(isset($label['label'])){
                                $resultData_image[$k_l]['label'][0] = $k_i .'-'.($k_l + 1). $resultData_image[$k_l]['label'][0];
                            }
                        }
                        //生成mark图
                        ...
                        $saveFile = $savePath;
                        if($image_folder){
                            $saveFile = $savePath . '/' . $image_folder;
                        }
                        $imageString = ImageHelper::image_get_content($resourceRoot . '/'.$image_path);
