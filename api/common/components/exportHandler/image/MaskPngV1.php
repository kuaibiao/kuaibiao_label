...
/**
 * 将图片标注结果导出成mask图(png)格式   清除锯齿，没有边缘渐变色
 */
class MaskPngV1 extends Component
{

    /**
     * 执行导出脚本，导出mask图(png)格式
    ...
    public static function run($args){
        ...
        #$batchDirname = 'mask_'.$projectId.'_'.implode(',', $batchIds).'_'.date('YmdHis');
        if(count($batchIds) > 3){
            $batchDirname = Yii::t('app', 'pack_script_mask_png_v1').'_'.$projectId.'_more_'.date('YmdHis');
        }else{
            $batchDirname = Yii::t('app', 'pack_script_mask_png_v1').'_'.$projectId.'_'.implode(',', $batchIds).'_'.date('YmdHis');
        }
        ...
        foreach ($groups as $i => $ids_)
        {
            ...
            foreach ($list as $v)
            {
                ...
                if (!isset($dataVal))
                {
                    ...
                    $checkMessages[$v['data_id']]['nodata'][]= Yii::t('app', 'pack_data_image_url_empty' );
                    continue;
                }
                //如果是http
                ...
                $options = ['antialias' => 0];
                $save_image_path = $item_save_path."/".$imageName.".png";
                $image = new ImageHandler();
                $image->create($imgData[0], $imgData[1], 'black');
                $image->mask($resultData, $options);
