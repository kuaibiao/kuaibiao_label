...
/**
 * 将图片标注结果导出成mark图且填充(jpg)格式
 */
class MarkJpgFill extends Component
{

    /**
     * 执行导出脚本，导出mark图且填充(jpg)格式
     * 打包数据检测结果和打包数据一起
     *
    ...
    public static function run($args){
        ...
        $batchDirname = Yii::t('app', 'pack_script_mark_image_fill').'_'.$projectId.'_'.implode(',', $batchIds).'_'.date('YmdHis');
        ...
        foreach ($groups as $i => $ids_)
        {
            ...
            foreach ($list as $v)
            {
                ...
                if(isset($configs['RemoveEmpty']) && $configs['RemoveEmpty'] && (!is_array($resultData)||count($resultData) ==  0 || (isset($resultData[0]['points']) && is_null($resultData[0]['points'][0]['x'])))){
                ...
                //生成mask图
                ImageProcessor::image_markFilled_from_string(
                    $imageName,
                    $imageString,
                    $resultData,
                    $item_save_path);
