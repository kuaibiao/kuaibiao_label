...
/**
 * 将图片标注结果导出成mark图无标签(jpg)格式
 */
class MarkJpgNoLabel extends Component
{
    ...
    public static function run($args){
        ...
        $batchDirname = Yii::t('app', 'pack_script_mark_image_without_tag').'_'.$projectId.'_'.implode(',', $batchIds).'_'.date('YmdHis');
        ...
        foreach ($groups as $i => $ids_)
        {
            ...
            foreach ($list as $v)
            {
                ...
                if(isset($config_arr['RemoveEmpty']) && $configs['RemoveEmpty'] && (!is_array($resultData)||count($resultData) ==  0 )){
                    $checkMessages['error'][$v['data_id']][]= "没有标注结果";
                    continue;
                }
                //生成mask图
