...
/**
 * 将图片标注结果导出成mask图(png)格式
 */
class MaskPng extends Component
{
    ...
    public static function run($args){
        ...
        $batchDirname = Yii::t('app', 'pack_script_mask_image').'_'.$projectId.'_'.implode(',', $batchIds).'_'.date('YmdHis');
        ...
        foreach ($groups as $i => $ids_)
        {
            ...
            foreach ($list as $v)
            {
                ...
                //生成mask图
                $imageNames = FileHelper::filebasename($imagepath);
                $imageNames = mb_substr($imageNames,0,strrpos($imageNames,'.'));
                $imageString = ImageHelper::image_get_content($imagepath);

                ImageProcessor::image_mask_from_string(
                    $imageNames,
                    $imageString,
                    $resultData,
                    $item_save_path,
