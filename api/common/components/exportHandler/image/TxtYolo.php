...
/**
 *
 * 2D的图片画框导出txt格式 YOLO（矩形框txt）
 *
 */
class TxtYolo extends Component
{
    ...
    public static function run($args)
    {
        ...
        if(count($batchIds) > 3){
            $batchDirname = Yii::t('app', 'pack_script_image_text_yolo').'_'.$projectId.'_more_'.date('YmdHis');
        }else{
            $batchDirname = Yii::t('app', 'pack_script_image_text_yolo').'_'.$projectId.'_'.implode(',', $batchIds).'_'.date('YmdHis');
        }
            ...
            foreach ($list as $k => $v)
            {
                ...
                if (is_array($resultData) && count($resultData)) {
                    ...
                    foreach ($resultData as  $k_l => $label)
                    {
                        if (!isset($label['type']))
                        {
                            $checkMessages['no_label_type'][]= $v['data_id'];
                            continue;
                        }
                        if($label['type']!='rect'){
                            $checkMessages['no_is_rect'][] = $v['data_id'];
                            continue;
                        }
                        $ot = trim(ProjectHandler::formatLabelValue($label['label']));
                        if(!$ot){
                            $ot = 0;
                        }
                        if($label['type'] != 'rect'){
                            $checkMessages['error'][$v['data_id']][] = ($k_l + 1) .'号框不是矩形框' . $label['type'];
                            continue;
                        }
                        //检查最小坐标点,最大坐标点
                        $center_x = 0;
                        ...
                        $center_x = $width / 2 + $left_top_point['x'];
                        $center_y = $height / 2 + $left_top_point['y'];
                        $txt .= $ot.' '.$center_x.' '.$center_y.' '.$width.' '.$height."\r\n";
                ...
                if(!empty($txt)){
                    $item_save_file = $savePath . '/' . str_replace('.json', '.txt', $jsonName);
                    FileHelper::file_write($item_save_file, $txt, 'w');
                    $succ_num++;
                }
