...
class NlpKeyword extends Component
{
        ...
        if ($dataArr)
        {
            foreach ($dataArr as $resourceKey => $resourceUrl)
            {
                ...
                if (FileHelper::is_text_by_ext($resourceUrl))
                {
                    ...
                    $requestParams['txt_url'] = Url::toRoute(['site/download-private-file', 'file' => StringHelper::base64_encode($resourceUrl), 't' => time()], true);;
...
*/
                        $keywords = [];
                        if (!empty($data['keywords']) && !empty($data['phrases']))
                        {
                            $keywords = array_merge($data['keywords'], $data['phrases']);
                        }
                        else if (!empty($data['keywords']))
                        {
                            $keywords = $data['keywords'];
                        }
                        else if (!empty($data['phrases']))
                        {
                            $keywords = $data['phrases'];
                        }
